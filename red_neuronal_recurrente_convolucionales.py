...
Original file is located at
    https://colab.research.google.com/drive/1j1OLbEFnErmBeANeMePyNJ9AZla0zkJg
"""
...
from keras.layers import Dense, Dropout, Flatten, Conv2D, MaxPooling2D, Input, ConvLSTM2D, Conv3D, BatchNormalization
...
#Conv3d, la salida es un cubo en la que tengamos otra secuencia que sera las prediccion.
...
def genera_peliculas(n_samples=1200, n_frames=15):
    row = 80
    col = 80
	# Creamos pelicula ruidosas y desplazadas con ruido.
	# En situaciones reales existira ruido.
...
# El modelo se comporta como un modelo convolucional simple con recurrencia.
seq = Sequential()
seq.add(ConvLSTM2D(filters=40, kernel_size=(3, 3),
                   input_shape=(None, 40, 40, 1), #Peliculas de 40x40 pixeles
...
seq.add(ConvLSTM2D(filters=40, kernel_size=(3, 3),
                   padding='same', return_sequences=True))
seq.add(BatchNormalization())
#Conv3D, porque queremos sacar la informacion en formato de secuencia.
#La obtiene una sequencia de entrada y como salida otra secuencia.
seq.add(Conv3D(filters=1, kernel_size=(3, 3, 3),
               activation='sigmoid',
               padding='same', data_format='channels_last'))
#loss='binary_crossentropy', La regresion es de valores binario. los valore del cubo estan en 0 o 1.
